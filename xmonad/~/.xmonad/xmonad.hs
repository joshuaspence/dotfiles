...
-- We define two lists of keycodes for use in the rest of the keyboard 
-- configuration. The first is the list of numpad keys, in the order they occur 
-- on the keyboard (left to right and top to bottom). The second is the list of 
-- number keys, in an order corresponding to the numpad. We will use these to
-- make workspace navigation commands work the same whether you use the numpad 
-- or the top-row number keys. And, we also use them to figure out where to go 
...
-- Here, some magic occurs that I once grokked but has since fallen out of my 
-- head. Essentially what is happening is that we are telling xmonad how to 
-- navigate workspaces, how to send windows to different workspaces, and what 
-- keys to use to change which monitor is focused.

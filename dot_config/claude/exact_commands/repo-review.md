...
- `--breadth <n|auto>` sets how many coherent review units the repository is partitioned into in step 3. Must be a
  ...
  the range 4–8.
- `--depth <n|auto>` sets how many independent validators run per issue in step 6. Must be a positive integer or
  `auto`; reject any other value and stop with an error rather than guessing. Defaults to `1`. With a fixed `n > 1`,
  keep an issue only if a strict majority of its validators confirm it
